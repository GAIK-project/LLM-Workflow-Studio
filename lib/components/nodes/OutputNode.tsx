...
import { ChevronDown, ChevronUp, FileJson, FileText, MessageSquare } from 'lucide-react';
...
const OutputNode = ({ data, isConnectable }: NodeProps) => {
  ...
  const formats = [
    { value: 'text', label: 'Text', icon: <FileText size={14} className="mr-1" /> },
    { value: 'json', label: 'JSON', icon: <FileJson size={14} className="mr-1" /> },
  ];
  ...
  return (
    <div className="shadow-sm rounded-md min-w-[14rem] w-full overflow-hidden">
      <div className="bg-amber-500 text-white p-2 flex items-center justify-between cursor-pointer rounded-t-md" 
          ...
          <div className="flex items-center text-xs text-muted-foreground">
            {selectedFormat === 'json' ? 
              <div className="flex items-center">
                ...
                <span className="break-words">Returns structured data using AI SDK</span>
              </div> :
              <div className="flex items-center">
                <FileText size={14} className="mr-1" />
                <span className="break-words">Returns plain text responses</span>
              </div>
            }
          </div>
        </div>
