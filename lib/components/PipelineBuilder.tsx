...
const PipelineBuilder = () => {
  ...
  const executePipeline = async () => {
    ...
    try {
      ...
      const systemPrompt = modelNode.data.systemPrompt || 'You are a helpful assistant.';
      ...
      setPipelineResults({ error: error.message });
    } finally {
  ...
  const renderResultContent = () => {
    ...
    const outputNodes = nodes.filter((node) => node.type === 'output');
    const isJsonOutput = outputNodes.length > 0 && outputNodes[0].data.format === 'json';
    ...
    return (
      <div className="absolute bottom-4 right-4 z-40 w-96 max-h-[500px] bg-card p-4 rounded-md shadow-md border border-border overflow-auto">
        ...
        <div className="border border-border rounded-md p-3 bg-background">
          {content ? (
            <pre className={`text-sm ${isJsonOutput ? 'font-mono bg-muted/20 p-2 rounded' : ''} whitespace-pre-wrap`}>
              {content}
            </pre>
          ) : (
            <div className="text-muted-foreground text-sm">No results available</div>
          )}
        </div>
      </div>
    );
      ...
      <div className="flex-1 h-full relative">
        ...
        <div ref={reactFlowWrapper} className="h-full w-full">
          ...
          >
            ...
            <Panel position="top-left" className="m-2 bg-card p-4 rounded-lg shadow-md border border-border">
